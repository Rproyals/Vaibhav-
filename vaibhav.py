...
bot = telebot.TeleBot('7280461707:AAHSdzRND9Vmv4tgTXCNUFQGrqYqagDKWlw')

# Admin user IDs
admin_id = ["7154407009","5924790094"]
...
def show_owner(message):
    response = "👑 Bot Owner: @@pharaohsmacky"  # Replace with the actual owner username
    bot.reply_to(message, response)
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @pharaohsmacky can run this command."
...
def clear_logs_handler(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "Only @pharaohsmacky can run this command.")
